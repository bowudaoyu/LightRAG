...
# 气泡话题池定义
BUBBLE_TOPICS = {
    "值多少钱": {"emoji": "💰", "weight": 0.9, "desc": "拍卖价格、估值、收藏价值"},
    "谁用过它": {"emoji": "👑", "weight": 0.8, "desc": "历史主人、使用者、收藏家"},
    "现代等价物": {"emoji": "🔄", "weight": 0.85, "desc": "类比现代物品，帮助理解用途"},
    "鉴定秘籍": {"emoji": "🕵️", "weight": 0.7, "desc": "如何辨别真假、看哪些细节"},
    "震惊冷知识": {"emoji": "😱", "weight": 0.9, "desc": "意想不到的事实、反常识"},
    "命运多舛": {"emoji": "💔", "weight": 0.75, "desc": "被盗、流失、回归的故事"},
    "名人八卦": {"emoji": "🎭", "weight": 0.85, "desc": "与名人相关的趣事、传闻"},
    "黑科技": {"emoji": "🔬", "weight": 0.8, "desc": "制作工艺之谜、超前技术"},
    "数字震撼": {"emoji": "📏", "weight": 0.7, "desc": "惊人的数量、尺寸、年代"},
    "世界之最": {"emoji": "🌍", "weight": 0.8, "desc": "最早、唯一、存世最少"},
    "审美吐槽": {"emoji": "🎨", "weight": 0.75, "desc": "有趣的审美评价、风格点评"},
    "实战能力": {"emoji": "⚔️", "weight": 0.8, "desc": "兵器的杀伤力、实用性"},
    "灵异传说": {"emoji": "👻", "weight": 0.6, "desc": "民间传说、神秘故事"},
    "原来这么用": {"emoji": "🍽️", "weight": 0.7, "desc": "容易误解的真实用途"},
    "拍照攻略": {"emoji": "📷", "weight": 0.5, "desc": "最佳拍摄角度和细节"},
}
...
def select_bubble_topics(artifact_type: str, num_topics: int = 3) -> list[dict]:
    """
    根据文物类型智能选择气泡话题

    Args:
        artifact_type: 文物类型（如"瓷器"、"青铜器"等）
        num_topics: 需要选择的话题数量，默认3个

    Returns:
        选中的话题列表，每个元素包含 type, emoji, desc
    """
    import random

    # 获取匹配规则，如果没有匹配则使用默认规则
    rules = ARTIFACT_TOPIC_RULES.get(artifact_type, ARTIFACT_TOPIC_RULES["default"])

    selected = []
    used_types = set()

    # 1. 先选必选话题
    for topic_type in rules.get("must", []):
        if len(selected) < num_topics and topic_type in BUBBLE_TOPICS:
            topic_info = BUBBLE_TOPICS[topic_type]
            selected.append({
                "type": topic_type,
    ...
    # 2. 从优选话题中按权重随机选择
    avoid_set = set(rules.get("avoid", []))
    prefer_pool = [t for t in rules.get("prefer", []) if t not in used_types and t not in avoid_set]

    while len(selected) < num_topics and prefer_pool:
        weights = [BUBBLE_TOPICS[t]["weight"] for t in prefer_pool]
        chosen = random.choices(prefer_pool, weights=weights, k=1)[0]
        topic_info = BUBBLE_TOPICS[chosen]
        selected.append({
            "type": chosen,
            "emoji": topic_info["emoji"],
            "desc": topic_info["desc"]
        ...
        prefer_pool.remove(chosen)

    # 3. 如果还不够，从通用池补充（排除已选和avoid的）
    if len(selected) < num_topics:
        all_topics = [t for t in BUBBLE_TOPICS.keys()
                     if t not in used_types and t not in avoid_set]
        remaining_count = min(num_topics - len(selected), len(all_topics))
        if remaining_count > 0:
            remaining = random.sample(all_topics, remaining_count)
            for t in remaining:
                topic_info = BUBBLE_TOPICS[t]
                selected.append({
                    "type": t,
                    "emoji": topic_info["emoji"],
                    "desc": topic_info["desc"]
...
def format_topic_pool_for_prompt(topics: list[dict]) -> str:
    """
    将选中的话题格式化为 prompt 中的话题池说明

    Args:
        topics: select_bubble_topics 返回的话题列表
    ...
    for i, topic in enumerate(topics, 1):
        lines.append(f"{i}. {topic['emoji']} **{topic['type']}**：{topic['desc']}")
    return "\n".join(lines)
...
# ============================================================================
# 新版智能气泡 Prompt（只生成 title，不生成 detail 以加速响应）
# ============================================================================
PROMPTS["artifact_bubble_response"] = """---角色---
...
---写作原则---
1. **title要像弹幕**：短、狠、有悬念，让人忍不住想点
2. **用"竟然"、"居然"、"原来"等词制造好奇**
3. **说人话**：避免"具有重要历史价值"这类官方废话
4. **有观点**：可以吐槽、可以惊叹，别当百科全书
5. **接地气**：多用现代类比，让年轻人也能秒懂
...
{{
  "bubbles": [
    {{"type": "实战能力", "emoji": "⚔️", "title": "削20层纸不卷刃"}},
    {{"type": "黑科技", "emoji": "🔬", "title": "防锈配方至今成谜"}},
    {{"type": "名人八卦", "emoji": "🎭", "title": "卧薪尝胆那位的剑"}}
  ]
}}
```
"""
...
# ============================================================================
# 气泡详情 Prompt（用户点击气泡后按需生成 detail）
# ============================================================================
PROMPTS["artifact_bubble_detail"] = """---角色---
你是博物馆文物的"八卦翻译官"，为用户解读文物背后的有趣故事。

---任务---
用户点击了一个气泡标题，请为这个标题生成详细内容。
...
---输出要求---
直接输出50-100字的详情内容，不要输出任何其他内容（不需要JSON格式，直接输出文字）。

---写作原则---
1. **要有料**：给出具体数字、故事、对比，不要空洞的描述
2. **说人话**：避免"具有重要历史价值"这类官方废话
3. **有观点**：可以吐槽、可以惊叹，别当百科全书
4. **接地气**：多用现代类比，让年轻人也能秒懂
5. **紧扣标题**：内容要与气泡标题呼应，解释标题背后的故事
...
输出：
1965年出土时，考古人员用它轻松划破20多层纸，2500年前的剑至今锋利无比。剑身的菱形暗纹不是装饰，是古人独创的复合金属工艺，让剑既硬又韧。
"""

# ============================================================================
# 智能气泡 Prompt（同时生成 title 和 detail，开发调试用）
# ============================================================================
PROMPTS["artifact_bubble_response_with_detail"] = """---角色---
你是博物馆文物的"八卦翻译官"，把枯燥的文物知识变成让人忍不住想点的气泡。
...
---写作原则---
1. **title要像弹幕**：短、狠、有悬念，让人忍不住想点
2. **detail要有料**：给出具体数字、故事、对比，紧扣标题
3. **用"竟然"、"居然"、"原来"等词制造好奇**
4. **说人话**：避免"具有重要历史价值"这类官方废话
5. **有观点**：可以吐槽、可以惊叹，别当百科全书
6. **接地气**：多用现代类比，让年轻人也能秒懂
...
{{
  "bubbles": [
    {{"type": "实战能力", "emoji": "⚔️", "title": "削20层纸不卷刃", "detail": "1965年出土时，考古人员用它轻松划破20多层纸，2500年前的剑至今锋利无比。剑身的菱形暗纹不是装饰，是古人独创的复合金属工艺，让剑既硬又韧。"}},
    {{"type": "黑科技", "emoji": "🔬", "title": "防锈配方至今成谜", "detail": "在地下埋了2400多年，出土时依然寒光闪闪。科学家发现剑身有一层含铬的氧化膜，但古人怎么掌握这种工艺的，至今没人能解释清楚。"}},
    {{"type": "名人八卦", "emoji": "🎭", "title": "卧薪尝胆那位的剑", "detail": "剑身刻着"越王鸠浅自作用剑"，鸠浅就是勾践。这把剑见证了他从亡国奴到复仇者的逆袭全程，堪称古代最励志的随身装备。"}}
  ]
}}
```
"""
